const updateCommentCount = (result, diagCommentDiv) => {
  diagCommentDiv.innerHTML = '';
  ...
  diagCommentDiv.appendChild(dialogCommentCount);
  ...
  diagCommentDiv.appendChild(commentBox);
};

export const dialogCommentCount = async (diagCommentSec, iteMid) => {
  try {
    const URL = `https://us-central1-involvement-api.cloudfunctions.net/capstoneApi/apps/WdJx4eRTz6sSqysQlKxy/comments?iteMid=${iteMid}`;
    ...
    diagCommentSec.innerHTML = '';

    return updateCommentCount(result, diagCommentSec);
  } catch (error) { return error; }
};

export const dialogCommentCount2 = async (dialogContentDiv, iteMid) => {
  try {
